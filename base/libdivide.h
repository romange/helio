...
/* libdivide may use the pmuldq (vector signed 32x32->64 mult instruction) which is in SSE 4.1.
 * However, signed multiplication can be emulated efficiently with unsigned multiplication, and
 * SSE 4.1 is currently rare, so it is OK to not turn this on */
#ifdef LIBDIVIDE_USE_SSE4_1
#include <smmintrin.h>
#endif

#ifdef __cplusplus
/* We place libdivide within the libdivide namespace, and that goes in an anonymous namespace so
 * that the functions are only visible to files that #include this header and don't get external
 * linkage.  At least that's the theory. */
namespace libdivide {
#endif

/* Explanation of "more" field: bit 6 is whether to use shift path.  If we are using the shift path,
bit 7 is whether the divisor is negative in the signed case; in the unsigned case it is 0.   Bits
0-4 is shift value (for shift path or mult path).  In 32 bit case, bit 5 is always 0.  We use bit 7
as the "negative divisor indicator" so that we can use sign extension to efficiently go to a
full-width -1.
...
#ifdef __cplusplus
/* In C++, we don't want our public functions to be static, because they are arguments to templates
 * and static functions can't do that.  They get internal linkage through virtue of the anonymous
 * namespace.  In C, they should be static. */
...
#if LIBDIVIDE_VC
  // 64 bit windows doesn't seem to have an implementation of any of these load intrinsics, and 32
  // bit Visual C++ crashes
...
static inline __m128i libdivide_s64_signbits(__m128i v) {
  // we want to compute v >> 63, that is, _mm_srai_epi64(v, 63).  But there is no 64 bit shift right
  // arithmetic instruction in SSE2.  So we have to fake it by first duplicating the high 32 bit
  // values, and then using a 32 bit shift.  Another option would be to use _mm_srli_epi64(v, 63)
  // and then subtract that from 0, but that approach appears to be substantially slower for unknown
  // reasons
...
static inline __m128i libdivide_s64_shift_right_vector(__m128i v, int amt) {
  // implementation of _mm_sra_epi64.  Here we have two 64 bit values which are shifted right to
  // logically become (64 - amt) values, and are then sign extended from a (64 - amt) bit number.
  const int b = 64 - amt;
...
/* Here, b is assumed to contain one 32 bit value repeated four times.  If it did not, the function
 * would not work. */
...
static inline __m128i libdivide_mullhi_u64_flat_vector(__m128i x, __m128i y) {
  ...
  const __m128i x0 = _mm_and_si128(x, mask),
                x1 = _mm_srli_epi64(
                    x, 32);  // x0 is low half of 2 64 bit values, x1 is high half in low slots
  const __m128i y0 = _mm_and_si128(y, mask), y1 = _mm_srli_epi64(y, 32);
  const __m128i x0y0_hi =
      _mm_srli_epi64(_mm_mul_epu32(x0, y0),
                     32);  // x0 happens to have the low half of the two 64 bit values in 32 bit
                           // slots 0 and 2, so _mm_mul_epu32 computes their full product, and then
                           // we shift right by 32 to get just the high values
...
/* SSE2 does not have a signed multiplication instruction, but we can convert unsigned to signed
 * pretty efficiently.  Again, b is just a 32 bit value repeated four times. */
static inline __m128i libdivide_mullhi_s32_flat_vector(__m128i a, __m128i b) {
...
static inline int32_t libdivide__count_trailing_zeros64(uint64_t val) {
  ...
  /* Fast way to count trailing zeros.  Note that we disable this in 32 bit because gcc does
   * something horrible - it calls through to a dynamically bound function. */
...
// libdivide_64_div_32_to_32: divides a 64 bit uint {u1, u0} by a 32 bit uint {v}.  The result must
// fit in 32 bits.  Returns the quotient directly and the remainder in *r
...
/* Code taken from Hacker's Delight, http://www.hackersdelight.org/HDcode/divlu.c .  License permits
 * inclusion here per http://www.hackersdelight.org/permissions.htm
 */
static uint64_t libdivide_128_div_64_to_64(uint64_t u1, uint64_t u0, uint64_t v, uint64_t* r) {
...
struct libdivide_u32_t libdivide_u32_gen(uint32_t d) {
  ...
  if ((d & (d - 1)) == 0) {
    ...
    if (e < (1U << floor_log_2_d)) {
      ...
      /* We have to use the general 33-bit algorithm.  We need to compute (2**power) / d. However,
    ...
    // subtract one from the shift because we're using the smaller power. If we're using the larger
    // power, we subtract one from the shift because it's taken care of by the add indicator.  So
    // floor_log_2_d happens to be correct in both cases.
  }
  return result;
}
...
struct libdivide_u64_t libdivide_u64_gen(uint64_t d) {
  ...
  if ((d & (d - 1)) == 0) {
    ...
    uint8_t more;
    proposed_m = libdivide_128_div_64_to_64(1ULL << floor_log_2_d, 0, d,
    ...
    // subtract one from the shift because we're using the smaller power. If we're using the larger
    // power, we subtract one from the shift because it's taken care of by the add indicator.  So
    // floor_log_2_d happens to be correct in both cases, which is why we do it outside of the if
    // statement.
  }
  return result;
}
...
struct libdivide_s32_t libdivide_s32_gen(int32_t d) {
   ...
   * important because the magic algorithm fails for -1.  To check if d is a power of 2 or its
   * inverse, it suffices to check whether its absolute value has exactly one bit set.  This works
   * even for INT_MIN, because abs(INT_MIN) == INT_MIN, and INT_MIN has one bit set and is a power
   * of 2.  */
  uint32_t absD = (uint32_t)(d < 0 ? -d : d);  // gcc optimizes this to the fast abs trick
  if ((absD & (absD - 1)) ==
      0) {  // check if exactly one bit is set, don't care if absD is 0 since that's divide by zero
    result.magic = 0;
    result.more = libdivide__count_trailing_zeros32(absD) |
                  (d < 0 ? LIBDIVIDE_NEGATIVE_DIVISOR : 0) | LIBDIVIDE_S32_SHIFT_PATH;
  } else {
    ...
    /* We are going to start with a power of floor_log_2_d - 1.  This works if works if e <
     * 2**floor_log_2_d. */
    if (e < (1U << floor_log_2_d)) {
      /* This power works */
      ...
      if (twice_rem >= absD || twice_rem < rem)
        proposed_m += 1;
      more = floor_log_2_d | LIBDIVIDE_ADD_MARKER |
             (d < 0 ? LIBDIVIDE_NEGATIVE_DIVISOR : 0);  // use the general algorithm
    }
    proposed_m += 1;
...
__m128i libdivide_s32_do_vector(__m128i numers, const struct libdivide_s32_t* denom) {
  ...
  if (more & LIBDIVIDE_S32_SHIFT_PATH) {
    ...
    __m128i roundToZeroTweak =
        _mm_set1_epi32((1 << shifter) - 1);  // could use _mm_srli_epi32 with an all -1 register
    ...
    __m128i shiftMask = _mm_set1_epi32(
        (int32_t)((int8_t)more >> 7));  // set all bits of shift mask = to the sign bit of more
    ...
    if (more & LIBDIVIDE_ADD_MARKER) {
      ...
      q = _mm_add_epi32(
          q, _mm_sub_epi32(_mm_xor_si128(numers, sign), sign));  // q += ((numer ^ sign) - sign);
    }
...
struct libdivide_s64_t libdivide_s64_gen(int64_t d) {
  ...
  if ((absD & (absD - 1)) ==
      0) {  // check if exactly one bit is set, don't care if absD is 0 since that's divide by zero
    result.more =
        libdivide__count_trailing_zeros64(absD) | (d < 0 ? LIBDIVIDE_NEGATIVE_DIVISOR : 0);
    result.magic = 0;
  } else {
    const uint32_t floor_log_2_d = 63 - libdivide__count_leading_zeros64(absD);

    // the dividend here is 2**(floor_log_2_d + 63), so the low 64 bit word is 0 and the high word
    // is floor_log_2_d - 1
    uint8_t more;
...
__m128i libdivide_s64_do_vector(__m128i numers, const struct libdivide_s64_t* denom) {
  ...
  if (magic == 0) {  // shift path
    ...
    if (more & LIBDIVIDE_ADD_MARKER) {
      __m128i sign = _mm_set1_epi32((int32_t)((int8_t)more >> 7));  // must be arithmetic shift
      q = _mm_add_epi64(
          q, _mm_sub_epi64(_mm_xor_si128(numers, sign), sign));  // q += ((numer ^ sign) - sign);
    }
