...
struct can_invoke<T, identity<Args...>, decltype((void)std::declval<T>()(std::declval<Args>()...))>
    : std::true_type {};
...
struct can_invoke<Pointer, identity<T&>,
                  decltype((void)(std::declval<T&>().*std::declval<Pointer>()))> : std::true_type {
};
template <typename Pointer, typename T>
struct can_invoke<Pointer, identity<T&&>,
                  decltype((void)(std::declval<T&&>().*std::declval<Pointer>()))> : std::true_type {
...
struct overload_impl<Current, Next, Rest...> : Current, overload_impl<Next, Rest...> {
  explicit overload_impl(Current current, Next next, Rest... rest)
      : Current(std::move(current)), overload_impl<Next, Rest...>(std::move(next),
                                                                  std::move(rest)...) {
  }
...
template <typename T, typename = void> struct address_taker {
  template <typename O> static void* take(O&& obj) {
...
template <typename T> struct address_taker<T, std::enable_if_t<std::is_pointer<T>::value>> {
  ...
  template <typename O> static T restore(O ptr) {
...
struct box_factory<box<IsCopyable, T, Allocator>> {
  using real_allocator = typename std::allocator_traits<
      std::decay_t<Allocator>>::template rebind_alloc<box<IsCopyable, T, Allocator>>;
...
template <typename From, typename To>
using transfer_volatile_t = std::conditional_t<std::is_volatile<std::remove_pointer_t<From>>::value,
...
struct invoke_table<First, Second, Args...> {
  using type = std::tuple<function_pointer_of<First>, function_pointer_of<Second>,
                          function_pointer_of<Args>...> const*;

  /// Return the function pointer at the particular index
  template <std::size_t Index> static constexpr auto fetch(type table) noexcept {
  ...
  struct empty_vtable : public std::tuple<function_pointer_of<First>, function_pointer_of<Second>,
                                          function_pointer_of<Args>...> {
...
#define FU2_DEFINE_FUNCTION_TRAIT(CONST, VOLATILE, NOEXCEPT, OVL_REF, REF)                         \
  template <std::size_t Index, typename Function, typename Ret, typename... Args, typename Next,   \
            typename... Signatures>                                                                \
  class operator_impl<Index, Function, Ret(Args...) CONST VOLATILE OVL_REF NOEXCEPT, Next,         \
                      Signatures...> : operator_impl<Index + 1, Function, Next, Signatures...> {   \
...
class vtable<property<IsThrowing, HasStrongExceptGuarantee, FormalArgs...>> {
  ...
  template <typename T> struct trait {
    ...
    static void process_cmd(vtable* to_table, opcode op, data_accessor* from,
                            std::size_t from_capacity, data_accessor* to, std::size_t to_capacity) {
      switch (op) {
        case opcode::op_move: {
          /// Retrieve the pointer to the object
          auto box = static_cast<T*>(
              retrieve<T>(std::integral_constant<bool, IsInplace>{}, from, from_capacity));
    ...
    template <typename Box>
    static void construct(std::false_type /*apply*/, Box&& /*box*/, vtable* /*to_table*/,
                          data_accessor* /*to*/,
                          std::size_t /*to_capacity*/) noexcept(HasStrongExceptGuarantee) {
    }
  };
 ...
 public:
  ...
  static void init(vtable& table, T&& object, data_accessor* to, std::size_t to_capacity) {
    trait<std::decay_t<T>>::construct(std::true_type{}, std::forward<T>(object), &table, to,
                                      to_capacity);
  }
  ...
  void copy(vtable& to_table, data_accessor const* from, std::size_t from_capacity,
            data_accessor* to, std::size_t to_capacity) const {
    cmd_(&to_table, opcode::op_copy, const_cast<data_accessor*>(from), from_capacity, to,
         to_capacity);
  }
  ...
  /// vtable
  void weak_destroy(data_accessor* from,
...
/// control and vtable.
template <typename Capacity, typename = void> struct internal_capacity {
...
template <typename Capacity> class internal_capacity_holder {
  // Tag to access the structure in a type-safe way
...
class erasure : internal_capacity_holder<typename Config::capacity> {
 ...
 public:
  ...
  constexpr erasure(erasure&& right) noexcept(Property::is_strong_exception_guaranteed) {
    right.vtable_.move(vtable_, right.opaque_ptr(), right.capacity(), this->opaque_ptr(),
                       capacity());
  }

  constexpr erasure(erasure const& right) {
    right.vtable_.copy(vtable_, right.opaque_ptr(), right.capacity(), this->opaque_ptr(),
                       capacity());
  }
  ...
  template <typename OtherConfig>
  constexpr erasure& operator=(erasure<true, OtherConfig, Property> right) noexcept(
      Property::is_strong_exception_guaranteed) {
    vtable_.weak_destroy(this->opaque_ptr(), capacity());
    right.vtable_.move(vtable_, right.opaque_ptr(), right.capacity(), this->opaque_ptr(),
                       capacity());
  ...
  static constexpr decltype(auto) invoke(Erasure&& erasure, Args&&... args) {
    ...
    return erasure.vtable_.template invoke<Index>(std::forward<Erasure>(erasure).opaque_ptr(),
                                                  capacity, std::forward<Args>(args)...);
  }
};
...
class erasure<false, Config, property<IsThrowing, HasStrongExceptGuarantee, Args...>> {
 ...
 public:
  ...
  template <typename T> constexpr void assign(std::false_type /*use_bool_op*/, T&& callable) {
    invoke_table_ = invoke_table_t::template get_invocation_view_table_of<std::decay_t<T>>();
    view_.ptr_ = address_taker<std::decay_t<T>>::take(std::forward<T>(callable));
  }
  template <typename T> constexpr void assign(std::true_type /*use_bool_op*/, T&& callable) {
    if (bool(callable)) {
...
template <typename T, typename Signature,
          typename Trait = type_erasure::invocation_table::function_trait<Signature>>
struct accepts_one
...
template <typename T> struct use_bool_op : std::false_type {};
#else
template <typename T, typename = void> struct has_bool_op : std::false_type {};
template <typename T>
...
template <typename T> struct use_bool_op : has_bool_op<T> {};
...
template <typename Ret, typename... Args> struct use_bool_op<Ret(Args...)> : std::false_type {};
...
template <bool IsStrongExceptGuaranteed, typename T> struct assert_no_strong_except_guarantee {
  static_assert(!IsStrongExceptGuaranteed || (std::is_nothrow_move_constructible<T>::value &&
...
using is_owning_correct =
    std::integral_constant<bool, (LeftConfig::is_owning == RightConfig::is_owning)>;
...
class function<Config, property<IsThrowing, HasStrongExceptGuarantee, Args...>>
 ...
 public:
  ...
  constexpr function(function<RightConfig, property_t> const& right) : erasure_(right.erasure_) {
  }
  ...
  template <typename T,  //
            ...
            enable_if_can_accept_all_t<T>* = nullptr, assert_wrong_copy_assign_t<T>* = nullptr,
  ...
  template <typename T, typename Allocator,  //
            enable_if_not_convertible_to_this<T>* = nullptr,
            enable_if_can_accept_all_t<T>* = nullptr, enable_if_owning_t<T>* = nullptr,
  ...
  using type_erasure::invocation_table::operator_impl<0U, function<Config, property_t>,
                                                      Args...>::operator();
};
...
template <typename T> struct capacity_can_hold {
...
using function_base =
    detail::function<detail::config<IsOwning, IsCopyable, Capacity>,
