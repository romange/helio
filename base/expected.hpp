...
//
// in_place: code duplicated in any-lite, expected-lite, expected-lite, value-ptr-lite,
// variant-lite:
//
...
nsel_DISABLE_MSVC_WARNINGS(26409)
  ...
  /// x.x.5 Unexpected object type; unexpected_type; C++17 and later can also use aliased type
  /// unexpected.
...
#endif  // nsel_P0323R
  {
   ...
   public:
    ...
    template <typename U> struct rebind { using type = expected<U, error_type>; };
    ...
    template <typename U,
              typename G nsel_REQUIRES_T(std::is_constructible<T, U const&>::value&&
    ...
    // Effects: If T is not cv void and is_trivially_destructible_v<T> is false and bool(*this),
    // calls val.~T(). If is_trivially_destructible_v<E> is false and !bool(*this), calls
    // unexpect.~unexpected<E>(). Remarks: If either T is cv void or is_trivially_destructible_v<T>
    // is true, and is_trivially_destructible_v<E> is true, then this destructor shall be a trivial
    // destructor.
