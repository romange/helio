...
*/
    ...
    (defined(__i386) || defined(__x86_64__) || defined(ARCH_K8))
...
class RWSpinLock {
 ...
 public:
  ...
  void lock_shared() {
    int count = 0;
    while (!PREDICT_TRUE(try_lock_shared())) {
      if (++count > 1000)
        sched_yield();
    }
  }
  ...
  bool try_lock_shared() {
    ...
    if (PREDICT_FALSE(value & (WRITER | UPGRADED))) {
      bits_.fetch_add(-READER, std::memory_order_release);
      return false;
    }
    return true;
 ...
 private:
  ...
  void operator=(const RWSpinLock&) = delete;
};
 ...
 public:
  ...
  void writeLockAggressive() {
    ...
    while (val != load_acquire(&ticket.write)) {
      asm volatile("pause");
      if (PREDICT_FALSE(++count > 1000)) sched_yield();
    }
  }
  ...
  void lock_shared() {
    ...
    while (!LIKELY(try_lock_shared())) {
      asm volatile("pause");
      if (PREDICT_FALSE((++count & 1023) == 0)) sched_yield();
    }
  }
