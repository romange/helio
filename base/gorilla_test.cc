...
TEST_F(GorillaTest, DoubleCompress) {
  std::vector<double> data = generateSequence(100.0, 10000.0);
  ASSERT_GT(data.size(), 1024);

  size_t inp_sz = data.size() * 8;
  vector<uint8_t> outp(inp_sz);
  DoubleCompressor compressor;
  compressor.Commit(data.data(), 1024, outp.data());
  #define BYTE(j) (int(outp[i * 8 + j]))

  for (unsigned i = 0; i < 1024; ++i) {
    LOG(INFO) << BYTE(0) << " " << BYTE(1) << " " << BYTE(2) << " "
