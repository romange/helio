...
error_code GcsWriteFile::Close() {
  ...
  if (!absl::GetFlag(FLAGS_gcs_dry_upload)) {
    ...
    io::Result<RobustSender::HeaderParserPtr> res = sender.Send(client_handle.get(), req.get());
    if (!res) {
...
io::Result<io::WriteFile*> OpenWriteGcsFile(const string& bucket, const string& key,
  ...
  io::Result<RobustSender::HeaderParserPtr> res = sender.Send(client_handle.get(), &empty_req);
  if (!res) {
