...
error_code GcsWriteFile::Close() {
  ...
  if (!absl::GetFlag(FLAGS_gcs_dry_upload)) {
    ...
    io::Result<RobustSender::SenderResult> res = sender.Send(3, req.get());
    if (!res) {
      LOG(ERROR) << "Error closing GCS file " << create_file_name() << " for request: \n"
                 << req->GetHeaders() << ", status " << res.error().message();
      return res.error();
    }
    h2::response_parser<h2::string_body> resp(std::move(*res->eb_parser));
    auto client_handle = std::move(res->client_handle);
...
error_code GcsWriteFile::Upload() {
  ...
  if (!absl::GetFlag(FLAGS_gcs_dry_upload)) {
    ...
    auto parser_ptr = std::move(res->eb_parser);
...
io::SizeOrError GcsReadFile::Read(size_t offset, const iovec* v, uint32_t len) {
  ...
  if (!client_handle_) {
    ...
    RobustSender sender(opts_.pool, opts_.creds_provider);
    io::Result<RobustSender::SenderResult> res = sender.Send(3, &empty_req);
    if (!res) {
      return nonstd::make_unexpected(res.error());
    }

    client_handle_ = std::move(res->client_handle);
    parser_.emplace(std::move(*res->eb_parser));
