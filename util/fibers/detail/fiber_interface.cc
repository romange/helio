...
ctx::fiber_context FiberInterface::SwitchTo() {
  ...
  return std::move(entry_).resume_with([this, prev, resume_hook, fake_stack_save](ctx::fiber_context&& c) {
    DCHECK(!prev->entry_);
...
void FiberInterface::SwitchToAndExecute(std::function<void()> fn) {
  ...
  // pass pointer to the context that resumes `this`
  std::move(entry_).resume_with([this, prev, resume_hook, fn = std::move(fn),
