...
TEST_P(FiberSocketTest, EpollShutdownWhileReading) {
  if (GetProactorType() != "epoll") {
    GTEST_SKIP() << "only relevant for epoll proactor";
    return;
  }
  ...
  proactor_->Dispatch([&] {
    ...
    EXPECT_EQ(ec.error(), std::make_error_code(errc::connection_aborted));
